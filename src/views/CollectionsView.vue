    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-12 border-b border-stone-200 pb-4 gap-4">
      <div class="flex items-center gap-4">
        ...
        <div v-if="activeFilters.length > 0" class="flex gap-2">
          <span v-for="filter in activeFilters" :key="filter" class="px-3 py-1 bg-stone-100 text-xs font-bold text-stone-600 rounded-full uppercase">
            {{ filter }}
      ...
      <div class="flex items-center gap-4">
        <select :value="sortBy" @change="updateSort" class="bg-transparent text-sm font-medium text-stone-900 uppercase tracking-widest focus:outline-none cursor-pointer">
