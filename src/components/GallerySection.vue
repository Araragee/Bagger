...
<template>
  <section ref="sectionRef" class="relative h-screen overflow-hidden bg-stone-100">
    <div ref="trackRef" class="flex h-full w-[500vw]">
      ...
      <!-- Panels 2-5: Products -->
      <div v-for="product in galleryItems" :key="product.id" class="relative flex h-full w-screen items-center justify-center bg-stone-50 p-10">
        <div class="flex h-full w-full max-w-6xl flex-col items-center justify-center gap-10 md:flex-row">
          <div class="h-[50vh] w-full md:w-1/2 overflow-hidden shadow-2xl">
          ...
          <div class="flex w-full flex-col items-start justify-center md:w-1/2">
            <span class="mb-2 text-sm font-bold uppercase tracking-widest text-amber-800">{{ product.category }}</span>
            <h3 class="mb-4 text-4xl font-serif text-stone-900 md:text-5xl">{{ product.name }}</h3>
            <p class="mb-6 text-lg text-stone-600">{{ product.description }}</p>
            <p class="text-2xl font-medium text-stone-900">{{ product.price }}</p>
            <button class="mt-8 rounded-none border border-stone-900 px-8 py-3 text-sm font-medium uppercase tracking-widest text-stone-900 transition-colors hover:bg-stone-900 hover:text-white">
              View Details
            </button>
          </div>
        </div>
